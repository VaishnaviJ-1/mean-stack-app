...
\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ type   }&\cellcolor{\tableheadbgcolor}\textbf{ description    }\\\cline{1-2}
\endhead
\PBS\centering $\ast$   &matches any type    \\\cline{1-2}
...
\PBS\centering E   &{\ttfamily instanceof Error} OR {\ttfamily null} $\ast$$\ast$(special\+: see below)$\ast$$\ast$    \\\cline{1-2}
...
If an {\bfseries{error}} argument is found and is not null then the remaining arguments are optional. That is, if you say {\ttfamily ESO} then that\textquotesingle{}s like using a non-\/magical {\ttfamily E} in\+: {\ttfamily E$\vert$\+ESO$\vert$\+ZSO}.\hypertarget{md__c___users_vaishnavi_jadhav__desktop__developer_code_mean_stack_example_client_node_modules_aproba__r_e_a_d_m_e_autotoc_md1990}{}\doxysubsection{But I have optional arguments?!}\label{md__c___users_vaishnavi_jadhav__desktop__developer_code_mean_stack_example_client_node_modules_aproba__r_e_a_d_m_e_autotoc_md1990}
You can provide more than one signature by separating them with pipes {\ttfamily $\vert$}. If any signature matches the arguments then they\textquotesingle{}ll be considered valid.
...
This would be a signature of {\ttfamily SO$\vert$S}. That is, a string and and object, or just a string.
...
To reproduce this you have to fully enumerate all of the possible combinations and that implies a signature of {\ttfamily SO$\vert$\+SS$\vert$\+OO$\vert$\+OS$\vert$\+S$\vert$O}. The awkwardness is a feature\+: It reminds you of the complexity you\textquotesingle{}re adding to your API when you do this sort of thing.\hypertarget{md__c___users_vaishnavi_jadhav__desktop__developer_code_mean_stack_example_client_node_modules_aproba__r_e_a_d_m_e_autotoc_md1991}{}\doxysubsection{Browser support}\label{md__c___users_vaishnavi_jadhav__desktop__developer_code_mean_stack_example_client_node_modules_aproba__r_e_a_d_m_e_autotoc_md1991}
This has no dependencies and should work in browsers, though you\textquotesingle{}ll have noisier stack traces.\hypertarget{md__c___users_vaishnavi_jadhav__desktop__developer_code_mean_stack_example_client_node_modules_aproba__r_e_a_d_m_e_autotoc_md1992}{}\doxysubsection{Why this exists}\label{md__c___users_vaishnavi_jadhav__desktop__developer_code_mean_stack_example_client_node_modules_aproba__r_e_a_d_m_e_autotoc_md1992}
