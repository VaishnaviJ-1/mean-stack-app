...
It also provides an abstraction for handling \href{https://html.spec.whatwg.org/multipage/comms.html\#server-sent-events}{\texttt{ Event\+Source}} connections, which are one-\/way connections that allow the server to push data to the client. They are based on streaming HTTP responses and can be easier to access via proxies than Web\+Sockets.\hypertarget{md__c___users_vaishnavi_jadhav__desktop__developer_code_mean_stack_example_client_node_modules_faye_websocket__r_e_a_d_m_e_autotoc_md5452}{}\doxysection{Installation}\label{md__c___users_vaishnavi_jadhav__desktop__developer_code_mean_stack_example_client_node_modules_faye_websocket__r_e_a_d_m_e_autotoc_md5452}

\begin{DoxyCode}{0}
\DoxyCodeLine{\$ npm install faye-\/websocket}

\end{DoxyCode}
\hypertarget{md__c___users_vaishnavi_jadhav__desktop__developer_code_mean_stack_example_client_node_modules_faye_websocket__r_e_a_d_m_e_autotoc_md5453}{}\doxysection{Handling Web\+Socket connections in Node}\label{md__c___users_vaishnavi_jadhav__desktop__developer_code_mean_stack_example_client_node_modules_faye_websocket__r_e_a_d_m_e_autotoc_md5453}
...
\end{DoxyCode}
\hypertarget{md__c___users_vaishnavi_jadhav__desktop__developer_code_mean_stack_example_client_node_modules_faye_websocket__r_e_a_d_m_e_autotoc_md5454}{}\doxysection{Using the Web\+Socket client}\label{md__c___users_vaishnavi_jadhav__desktop__developer_code_mean_stack_example_client_node_modules_faye_websocket__r_e_a_d_m_e_autotoc_md5454}
...
The {\ttfamily tls} value is an object that will be passed to \href{https://nodejs.org/api/tls.html\#tls_tls_connect_options_callback}{\texttt{ {\ttfamily tls.\+connect()}}}.\hypertarget{md__c___users_vaishnavi_jadhav__desktop__developer_code_mean_stack_example_client_node_modules_faye_websocket__r_e_a_d_m_e_autotoc_md5455}{}\doxysection{Subprotocol negotiation}\label{md__c___users_vaishnavi_jadhav__desktop__developer_code_mean_stack_example_client_node_modules_faye_websocket__r_e_a_d_m_e_autotoc_md5455}
...
If the client and server agree on a protocol, both the client-\/ and server-\/side socket objects expose the selected protocol through the {\ttfamily ws.\+protocol} property.\hypertarget{md__c___users_vaishnavi_jadhav__desktop__developer_code_mean_stack_example_client_node_modules_faye_websocket__r_e_a_d_m_e_autotoc_md5456}{}\doxysection{Protocol extensions}\label{md__c___users_vaishnavi_jadhav__desktop__developer_code_mean_stack_example_client_node_modules_faye_websocket__r_e_a_d_m_e_autotoc_md5456}
...
\end{DoxyCode}
\hypertarget{md__c___users_vaishnavi_jadhav__desktop__developer_code_mean_stack_example_client_node_modules_faye_websocket__r_e_a_d_m_e_autotoc_md5457}{}\doxysection{Initialization options}\label{md__c___users_vaishnavi_jadhav__desktop__developer_code_mean_stack_example_client_node_modules_faye_websocket__r_e_a_d_m_e_autotoc_md5457}
...
\end{DoxyItemize}\hypertarget{md__c___users_vaishnavi_jadhav__desktop__developer_code_mean_stack_example_client_node_modules_faye_websocket__r_e_a_d_m_e_autotoc_md5458}{}\doxysection{Web\+Socket API}\label{md__c___users_vaishnavi_jadhav__desktop__developer_code_mean_stack_example_client_node_modules_faye_websocket__r_e_a_d_m_e_autotoc_md5458}
Both server-\/ and client-\/side {\ttfamily Web\+Socket} objects support the following API.


\begin{DoxyItemize}
\item $\ast$$\ast$`on(\textquotesingle{}open', function(event) \{\})\`{}$\ast$$\ast$ fires when the socket connection is established. Event has no attributes.
\item $\ast$$\ast$`on(\textquotesingle{}message', function(event) \{\}){\ttfamily $\ast$$\ast$ fires when the socket receives a message. Event has one attribute, $\ast$$\ast$}data{\ttfamily $\ast$$\ast$, which is either a}String$<$tt$>$(for   text frames) or aBuffer$<$tt$>$(for binary frames).
\item $\ast$$\ast$on(\textquotesingle{}error\textquotesingle{}, function(event) \{\})\`{}$\ast$$\ast$ fires when there is a protocol error due to bad data sent by the other peer. This event is purely informational, you do not need to implement error recover.
\item $\ast$$\ast$`on(\textquotesingle{}close', function(event) \{\}){\ttfamily $\ast$$\ast$ fires when either the client or the server closes the connection. Event has two optional attributes, $\ast$$\ast$}code{\ttfamily $\ast$$\ast$ and $\ast$$\ast$}reason\`{}$\ast$$\ast$, that expose the status code and message sent by the peer that closed the connection.
\item $\ast$$\ast${\ttfamily send(message)}$\ast$$\ast$ accepts either a {\ttfamily String} or a {\ttfamily Buffer} and sends a text or binary message over the connection to the other peer.
\item $\ast$$\ast${\ttfamily ping(message, function() \{\})}$\ast$$\ast$ sends a ping frame with an optional message and fires the callback when a matching pong is received.
\item $\ast$$\ast${\ttfamily close(code, reason)}$\ast$$\ast$ closes the connection, sending the given status code and reason text, both of which are optional.
\item $\ast$$\ast${\ttfamily version}$\ast$$\ast$ is a string containing the version of the {\ttfamily Web\+Socket} protocol the connection is using.
\item $\ast$$\ast${\ttfamily protocol}$\ast$$\ast$ is a string (which may be empty) identifying the subprotocol the socket is using.
\end{DoxyItemize}\hypertarget{md__c___users_vaishnavi_jadhav__desktop__developer_code_mean_stack_example_client_node_modules_faye_websocket__r_e_a_d_m_e_autotoc_md5459}{}\doxysection{Handling Event\+Source connections in Node}\label{md__c___users_vaishnavi_jadhav__desktop__developer_code_mean_stack_example_client_node_modules_faye_websocket__r_e_a_d_m_e_autotoc_md5459}
...
\begin{DoxyItemize}
\item $\ast$$\ast${\ttfamily url}$\ast$$\ast$ is a string containing the URL the client used to create the Event\+Source.
\item $\ast$$\ast${\ttfamily last\+Event\+Id}$\ast$$\ast$ is a string containing the last event ID received by the client. You can use this when the client reconnects after a dropped connection to determine which messages need resending.
\end{DoxyItemize}
...
\begin{DoxyItemize}
\item $\ast$$\ast${\ttfamily headers}$\ast$$\ast$ is an object containing custom headers to be set on the Event\+Source response.
\item $\ast$$\ast${\ttfamily retry}$\ast$$\ast$ is a number that tells the client how long (in seconds) it should wait after a dropped connection before attempting to reconnect.
\item $\ast$$\ast${\ttfamily ping}$\ast$$\ast$ is a number that tells the server how often (in seconds) to send \textquotesingle{}ping\textquotesingle{} packets to the client to keep the connection open, to defeat timeouts set by proxies. The client will ignore these messages.
\end{DoxyItemize}
