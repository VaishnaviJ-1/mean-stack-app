...
<title>My Project: Immutable collections for JavaScript</title>
...
<div><div class="header">
  <div class="headertitle"><div class="title">Immutable collections for JavaScript </div></div>
</div><!--header-->
<div class="contents">
<div class="textblock"><p ><a href="https://github.com/immutable-js/immutable-js/actions/workflows/ci.yml?query=branch%3Amain"><img src="https://github.com/immutable-js/immutable-js/actions/workflows/ci.yml/badge.svg?branch=main" alt="Build Status" style="pointer-events: none;" class="inline"/></a> <a href="https://immutable-js.slack.com">Chat on slack</a></p>
...
<h1><a class="anchor" id="autotoc_md6587"></a>
Introduction</h1>
...
<p >Immutable.js also provides a lazy <code>Seq</code>, allowing efficient chaining of collection methods like <code>map</code> and <code>filter</code> without creating intermediate representations. Create some <code>Seq</code> with <code>Range</code> and <code>Repeat</code>.</p>
...
<div class="fragment"><div class="line">npm install immutable</div>
</div><!-- fragment --><p >Or install using yarn.</p>
<div class="fragment"><div class="line">yarn add immutable</div>
</div><!-- fragment --><p >Then require it into any module.</p>
<div class="fragment"><div class="line">const { Map } = require(&#39;immutable&#39;);</div>
...
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md6589"></a>
Browser</h2>
<p >Immutable.js has no dependencies, which makes it predictable to include in a Browser.</p>
<p >It's highly recommended to use a module bundler like <a href="https://webpack.github.io/">webpack</a>, <a href="https://rollupjs.org/">rollup</a>, or <a href="https://browserify.org/">browserify</a>. The <code>immutable</code> npm module works without any additional consideration. All examples throughout the documentation will assume use of this kind of tool.</p>
...
<p >Use a script tag to directly add <code>Immutable</code> to the global scope:</p>
...
<p >Installing <code>immutable</code> via npm brings with it type definitions for Flow (v0.55.0 or higher) and TypeScript (v2.1.0 or higher), so you shouldn't need to do anything at all!</p>
<h3><a class="anchor" id="autotoc_md6591"></a>
Using TypeScript with Immutable.js v4</h3>
<p >Immutable.js type definitions embrace ES2015. While Immutable.js itself supports legacy browsers and environments, its type definitions require TypeScript's 2015 lib. Include either <code>"target": "es2015"</code> or <code>"lib": "es2015"</code> in your <code>tsconfig.json</code>, or provide <code>--target es2015</code> or <code>--lib es2015</code> to the <code>tsc</code> command.</p>
<div class="fragment"><div class="line">const { Map } = require(&#39;immutable&#39;);</div>
<div class="line">const map1 = Map({ a: 1, b: 2, c: 3 });</div>
<div class="line">const map2 = map1.set(&#39;b&#39;, 50);</div>
<div class="line">map1.get(&#39;b&#39;) + &#39; vs. &#39; + map2.get(&#39;b&#39;); // 2 vs. 50</div>
</div><!-- fragment --><h3><a class="anchor" id="autotoc_md6592"></a>
...
<div class="line">import Immutable from &#39;immutable&#39;;</div>
<div class="line">var map1: Immutable.Map&lt;string, number&gt;;</div>
<div class="line">map1 = Immutable.Map({ a: 1, b: 2, c: 3 });</div>
<div class="line">var map2 = map1.set(&#39;b&#39;, 50);</div>
<div class="line">map1.get(&#39;b&#39;); // 2</div>
<div class="line">map2.get(&#39;b&#39;); // 50</div>
</div><!-- fragment --><h1><a class="anchor" id="autotoc_md6593"></a>
...
</div><!-- fragment --><h1><a class="anchor" id="autotoc_md6594"></a>
...
<p >The difference for the immutable collections is that methods which would mutate the collection, like <code>push</code>, <code>set</code>, <code>unshift</code> or <code>splice</code>, instead return a new immutable collection. Methods which return new arrays, like <code>slice</code> or <code>concat</code>, instead return new immutable collections.</p>
<div class="fragment"><div class="line">const { List } = require(&#39;immutable&#39;);</div>
...
</div><!-- fragment --><p >Almost all of the methods on <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array">Array</a> will be found in similar form on <code>Immutable.List</code>, those of <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Map">Map</a> found on <code>Immutable.Map</code>, and those of <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Set">Set</a> found on <code>Immutable.Set</code>, including collection operations like <code>forEach()</code> and <code>map()</code>.</p>
<div class="fragment"><div class="line">const { Map } = require(&#39;immutable&#39;);</div>
...
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md6595"></a>
Convert from raw JavaScript objects and arrays.</h2>
<p >Designed to inter-operate with your existing JavaScript, Immutable.js accepts plain JavaScript Arrays and Objects anywhere a method expects a <code>Collection</code>.</p>
<div class="fragment"><div class="line">const { Map, List } = require(&#39;immutable&#39;);</div>
...
<div class="fragment"><div class="line">const { Seq } = require(&#39;immutable&#39;);</div>
...
<div class="fragment"><div class="line">const { fromJS } = require(&#39;immutable&#39;);</div>
<div class="line"> </div>
...
</div><!-- fragment --><p >Property access for JavaScript Objects first converts the key to a string, but since Immutable Map keys can be of any type the argument to <code>get()</code> is not altered.</p>
<h2><a class="anchor" id="autotoc_md6596"></a>
Converts back to raw JavaScript objects.</h2>
<p >All Immutable.js Collections can be converted to plain JavaScript Arrays and Objects shallowly with <code>toArray()</code> and <code>toObject()</code> or deeply with <code>toJS()</code>. All Immutable Collections also implement <code>toJSON()</code> allowing them to be passed to <code>JSON.stringify</code> directly. They also respect the custom <code>toJSON()</code> methods of nested objects.</p>
<div class="fragment"><div class="line">const { Map, List } = require(&#39;immutable&#39;);</div>
...
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md6597"></a>
Embraces ES2015</h2>
...
</div><!-- fragment --><p >Note: A Collection is always iterated in the same order, however that order may not always be well defined, as is the case for the <code>Map</code> and <code>Set</code>.</p>
<h1><a class="anchor" id="autotoc_md6598"></a>
...
</div><!-- fragment --><p >A few power-tools allow for reading and operating on nested data. The most useful are <code>mergeDeep</code>, <code>getIn</code>, <code>setIn</code>, and <code>updateIn</code>, found on <code>List</code>, <code>Map</code> and <code>OrderedMap</code>.</p>
<div class="fragment"><div class="line">const { fromJS } = require(&#39;immutable&#39;);</div>
<div class="line">const nested = fromJS({ a: { b: { c: [3, 4, 5] } } });</div>
<div class="line"> </div>
...
</div><!-- fragment --><h1><a class="anchor" id="autotoc_md6599"></a>
Equality treats Collections as Values</h1>
<p >Immutable.js collections are treated as pure data <em>values</em>. Two immutable collections are considered <em>value equal</em> (via <code>.equals()</code> or <code>is()</code>) if they represent the same collection of values. This differs from JavaScript's typical <em>reference equal</em> (via <code>===</code> or <code>==</code>) for Objects and Arrays which only determines if two variables represent references to the same object instance.</p>
<p >Consider the example below where two identical <code>Map</code> instances are not <em>reference equal</em> but are <em>value equal</em>.</p>
...
<div class="line"> </div>
<div class="line">const { Map, is } = require(&#39;immutable&#39;);</div>
<div class="line">const map1 = Map({ a: 1, b: 2, c: 3 });</div>
<div class="line">const map2 = Map({ a: 1, b: 2, c: 3 });</div>
...
</div><!-- fragment --><p >Value equality allows Immutable.js collections to be used as keys in Maps or values in Sets, and retrieved with different but equivalent collections:</p>
<div class="fragment"><div class="line">const { Map, Set } = require(&#39;immutable&#39;);</div>
<div class="line">const map1 = Map({ a: 1, b: 2, c: 3 });</div>
<div class="line">const map2 = Map({ a: 1, b: 2, c: 3 });</div>
<div class="line">const set = Set().add(map1);</div>
<div class="line">set.has(map2); // true because these are value-equal</div>
</div><!-- fragment --><p >Note: <code>is()</code> uses the same measure of equality as <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/is">Object.is</a> for scalar strings and numbers, but uses value equality for Immutable collections, determining if both are immutable and all keys and values are equal using the same measure of equality.</p>
...
<p >When comparing two collections, value equality may require considering every item in each collection, on an <code>O(N)</code> time complexity. For large collections of values, this could become a costly operation. Though if the two are not equal and hardly similar, the inequality is determined very quickly. In contrast, when comparing two collections with reference equality, only the initial references to memory need to be compared which is not based on the size of the collections, which has an <code>O(1)</code> time complexity. Checking reference equality is always very fast, however just because two collections are not reference-equal does not rule out the possibility that they may be value-equal.</p>
...
</div><!-- fragment --><h1><a class="anchor" id="autotoc_md6602"></a>
Batching Mutations</h1>
<blockquote class="doxtable">
<p >&zwj;If a tree falls in the woods, does it make a sound?</p>
...
<p>Applying a mutation to create a new immutable object results in some overhead, which can add up to a minor performance penalty. If you need to apply a series of mutations locally before returning, Immutable.js gives you the ability to create a temporary mutable (transient) copy of a collection and apply a batch of mutations in a performant manner by using <code>withMutations</code>. In fact, this is exactly how Immutable.js applies complex mutations itself.</p>
<p >As an example, building <code>list2</code> results in the creation of 1, not 3, new immutable Lists.</p>
<div class="fragment"><div class="line">const { List } = require(&#39;immutable&#39;);</div>
<div class="line">const list1 = List([1, 2, 3]);</div>
...
</div><!-- fragment --><p >Note: Immutable.js also provides <code>asMutable</code> and <code>asImmutable</code>, but only encourages their use when <code>withMutations</code> will not suffice. Use caution to not return a mutable copy, which could result in undesired behavior.</p>
<p ><em>Important!</em>: Only a select few methods can be used in <code>withMutations</code> including <code>set</code>, <code>push</code> and <code>pop</code>. These methods can be applied directly against a persistent data-structure where other methods like <code>map</code>, <code>filter</code>, <code>sort</code>, and <code>splice</code> will always return new immutable data-structures and never mutate a mutable collection.</p>
<h1><a class="anchor" id="autotoc_md6603"></a>
Lazy Seq</h1>
<p ><code>Seq</code> describes a lazy operation, allowing them to efficiently chain use of all the higher-order collection methods (such as <code>map</code> and <code>filter</code>) by not creating intermediate collections.</p>
<p ><b>Seq is immutable</b> — Once a Seq is created, it cannot be changed, appended to, rearranged or otherwise modified. Instead, any mutative method called on a <code>Seq</code> will return a new <code>Seq</code>.</p>
<p ><b>Seq is lazy</b> — <code>Seq</code> does as little work as necessary to respond to any method call. Values are often created during iteration, including implicit iteration when reducing or converting to a concrete data structure such as a <code>List</code> or JavaScript <code>Array</code>.</p>
<p >For example, the following performs no work, because the resulting <code>Seq</code>'s values are never iterated:</p>
<div class="fragment"><div class="line">const { Seq } = require(&#39;immutable&#39;);</div>
<div class="line">const oddSquares = Seq([1, 2, 3, 4, 5, 6, 7, 8])</div>
<div class="line">  .filter(x =&gt; x % 2 !== 0)</div>
<div class="line">  .map(x =&gt; x * x);</div>
</div><!-- fragment --><p >Once the <code>Seq</code> is used, it performs only the work necessary. In this example, no intermediate arrays are ever created, filter is called three times, and map is only called once:</p>
<div class="fragment"><div class="line">oddSquares.get(1); // 9</div>
</div><!-- fragment --><p >Any collection can be converted to a lazy Seq with <code>Seq()</code>.</p>
<div class="fragment"><div class="line">const { Map, Seq } = require(&#39;immutable&#39;);</div>
<div class="line">const map = Map({ a: 1, b: 2, c: 3 });</div>
<div class="line">const lazySeq = Seq(map);</div>
</div><!-- fragment --><p ><code>Seq</code> allows for the efficient chaining of operations, allowing for the expression of logic that can otherwise be very tedious:</p>
...
</div><!-- fragment --><p >As well as expressing logic that would otherwise seem memory or time limited, for example <code>Range</code> is a special kind of Lazy sequence.</p>
<div class="fragment"><div class="line">const { Range } = require(&#39;immutable&#39;);</div>
...
</div><!-- fragment --><h1><a class="anchor" id="autotoc_md6604"></a>
...
<h1><a class="anchor" id="autotoc_md6605"></a>
Contributing</h1>
...
<h2><a class="anchor" id="autotoc_md6607"></a>
License</h2>
